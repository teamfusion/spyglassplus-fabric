...
public class SpyglassStandEntity extends LivingEntity implements ScopingEntity {
    ...
    public static final EntityDimensions
        ...
        SMALL_DIMENSIONS = SpyglassPlusEntityType.SPYGLASS_STAND.getDimensions().scaled(0.5f);
    ...
    public SpyglassStandEntity(World world, double x, double y, double z) {
        this(SpyglassPlusEntityType.SPYGLASS_STAND, world);
    ...
    public ActionResult interact(PlayerEntity player, Hand hand) {
        ...
        if (spyglassStack.isEmpty()) {
            ...
            if (stack.getItem() instanceof ISpyglass spyglass) { // add spyglass stack
                ...
                this.setSmall(small);
                this.playSound(small ? SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_SHRINK : SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_ENLARGE, 1.0F, 1.0F);
                return ActionResult.SUCCESS;
            }
        } else {
            if (spyglassStack.getItem() instanceof ISpyglass spyglass) {
                if (player.isSneaking()) { // remove spyglass stack
                    ...
                    if (this.isOnGround() && player.isOnGround() && this.isWithinUseRange(player)) {
                        ...
                        player.setSpyglassStandEntity(this);
    ...
    public boolean doesNotMatch(PlayerEntity player) {
        return player.getSpyglassStandEntity().orElse(null) != this;
    }
    ...
    public void handleStatus(byte status) {
        if (status == EntityStatuses.HIT_ARMOR_STAND) {
            if (this.world.isClient) {
                this.world.playSound(this.getX(), this.getY(), this.getZ(), SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_HIT, this.getSoundCategory(), 0.3f, 1.0f, false);
    ...
    protected SoundEvent getHurtSound(DamageSource source) {
        return SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_HIT;
    }

    @Override
    protected SoundEvent getDeathSound() {
        return SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_BREAK;
    }
    ...
    public ItemStack getPickBlockStack() {
        ...
        return stack.isEmpty() ? new ItemStack(SpyglassPlusItems.SYPGLASS_STAND) : stack.copy();
    }
    ...
    protected void breakAndDropItem(DamageSource source) {
        Block.dropStack(this.world, this.getBlockPos(), new ItemStack(SpyglassPlusItems.SYPGLASS_STAND));
        this.onBreak(source);
    }
    ...
    protected void playBreakSound() {
        this.world.playSound(null, this.getX(), this.getY(), this.getZ(), SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_BREAK, this.getSoundCategory(), 1.0f, 1.0f);
    }

    protected void spawnBreakParticles() {
        if (this.world instanceof ServerWorld serverWorld) {
            serverWorld.spawnParticles(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(SpyglassPlusItems.SYPGLASS_STAND)), this.getX(), this.getBodyY(0.6D), this.getZ(), 10, this.getWidth() / 4.0f, this.getHeight() / 4.0f, this.getWidth() / 4.0f, 0.05);
        }
    }
    ...
    public LivingEntity.FallSounds getFallSounds() {
        return new LivingEntity.FallSounds(SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_FALL, SpyglassPlusSoundEvents.ENTITY_SPYGLASS_STAND_FALL);
    }
